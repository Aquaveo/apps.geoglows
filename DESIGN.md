...
- Serif display headings (Playfair Display) for editorial authority; Inter body for UI clarity
- Blue accent reserved for interactive affordances only; headings and text use dark neutrals
- Purposeful scroll animations via anime.js; respects prefers-reduced-motion
...
### Primary
- **Workbench Blue** (#2563eb): Buttons, links, focus rings, and interactive accent states. Never on headings or body text. Its restraint is the point.
...
- **Text Muted** (#94a3b8 light / #475569 dark): Labels, captions, metadata.
- **Border** (#e2e8f0 light / rgba(255,255,255,0.1) dark): Card edges, separators.

### Named Rules
**The Quiet Chrome Rule.** The portal frame (header, footer, navigation) uses only neutrals and the primary blue on interactive elements. Headings are dark slate, not blue. The tool screenshots and satellite imagery provide all the color the page needs.
...
**Character:** High-contrast serif headings create editorial authority and institutional credibility. Inter body text stays invisible, letting the content and data carry the reader. The pairing says "scientific journal" more than "SaaS product."
...
### Named Rules
**The Two-Voice Rule.** Playfair Display speaks only in headings (h1, h2, h3, card titles, modal titles). Inter handles everything else. No third typeface. The sign-in modal title also uses Playfair Display for consistency across the auth surface.

## 4. Elevation

Flat by default. Shadows appear as a response to state (hover, modal elevation), never as resting decoration. Dark mode uses borders instead of shadows for layering.
...
- **Editorial overlap** (`shadow-xl`): Used on the GRACE map image when overlapping the HydroSOS map in the hero bridge. Creates depth in the layered editorial composition.

### Named Rules
**The Flat-By-Default Rule.** Surfaces are flat at rest. Shadows appear only as a response to state (hover, focus, modal elevation) or in editorial compositions where images overlap intentionally.

## 5. Components

### Header
Top-center layout. GEOGLOWS droplet icon + wordmark centered; nav links ("App Library") and auth action positioned absolute top-right. Full-width with backdrop blur (bg-white/80 light, bg-slate-950/80 dark). Bottom border separator. Padding: py-8 mobile, py-12 desktop.

### Hero Bridge (anonymous visitors only)
Satellite photograph (aerial braided river delta) spanning full content width at 50-65vh height. Dark gradient overlay (bottom-to-top) carries the tagline in Playfair Display and supporting stats in Inter. Scales in on scroll via anime.js.

Below: two editorial images (HydroSOS global map + GRACE groundwater map) in an overlapping composition. HydroSOS at 82% width slides from left; GRACE at 55% width overlaps its bottom-right and slides from right. Both in 16:9 aspect-ratio containers with object-cover.

### App Showcases
Alternating layout: images left / text right, then reversed. Each showcase is a scroll section (min-h-80vh) with anime.js directional slide animations matching the layout direction. Images slide from the image side; text slides from the opposite side.
...
### Buttons
- **Primary:** Workbench Blue (#2563eb) background, white text, rounded-xl (0.75rem), px-4 py-2, Inter semibold. Hover: blue-700. Disabled: opacity-60. Focus-visible: 2px blue ring.
...
### Footer
Theme-aware GEOGLOWS logo (color for light, white for dark) at w-72. Tagline, pill-style external links (geoglows.org, Training). Contributors and sponsors as plain text. Copyright line with top border separator. Condensed spacing (mt-12 pt-10 pb-8).
...
### Do:
- **Do** use Playfair Display for headings only (h1, h2, h3, card titles, modal titles). Inter for everything else.
- **Do** reserve blue-600 for interactive elements (buttons, links, focus rings). Headings use dark slate.
- **Do** use real tool output (screenshots, satellite imagery) as the primary visual content. The data IS the design.
- **Do** respect prefers-reduced-motion: disable all anime.js animations, show content immediately.
