...
function renderAccountPanel(user, account) {
  if (!user) return "";
  ...
  const email = account?.profile?.email || user.profile?.email || "";
    ...
    <section class="mb-8">
      <div class="glass-card rounded-2xl p-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 class="text-xl font-bold text-slate-800 dark:text-white">${displayName}</h2>
            <p class="text-sm text-slate-600 dark:text-slate-400">${email}</p>
            ${
              account?.activeOrg
                ? `
              <div class="mt-3 flex flex-wrap items-center gap-2">
                <span class="text-xs font-medium px-2.5 py-1 rounded-full bg-blue-50 dark:bg-slate-800 text-blue-700 dark:text-slate-300 border border-blue-200 dark:border-slate-700">
                  Active org: ${account.activeOrg.name}
                </span>
                ${
                  activeRole
                    ? `
                  <span class="text-xs font-medium px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                    Role: ${activeRole}
                  </span>
                `
                    : ""
          ...
          <div class="flex flex-col gap-3 min-w-[260px]">
            ${
              ...
              <select
                id="orgSelector"
                class="px-3 py-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200"
              >
                ${orgs
                  .map(
                    (org) => `
            ...
            `
                : `
              <form id="createOrgForm" class="flex flex-col gap-2">
                <label class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                  Create organization
                </label>
                <input
  ...
  document.querySelector("#app").innerHTML = `
    <div class="min-h-screen text-slate-800 dark:text-slate-200 water-mesh flex flex-col">
      ...
      <main class="max-w-7xl mx-auto px-6 py-10 grow">
        ${renderAccountPanel(user, account)}
...
async function initApp() {
  ...
  if (user) {
    try {
      ...
      console.error("Profile/account bootstrap failed:", error);
    }
  }
